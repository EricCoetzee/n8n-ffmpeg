...
$action = {
    ...
    # --- CHANGED SECTION START: INPUT FULL FILENAME ---
    Write-Host "`n[INPUT] Enter the FULL desired filename:" -ForegroundColor Cyan
    Write-Host "   (Leave empty to keep original name: $originalName)" -ForegroundColor Gray
    Write-Host -NoNewline "   Filename > " -ForegroundColor Yellow
    
    $userFilename = Read-Host
    
    # 1. Clean invalid chars
    $invalidChars = [IO.Path]::GetInvalidFileNameChars()
    $cleanName = $userFilename
    foreach ($char in $invalidChars) {
        $cleanName = $cleanName.Replace($char.ToString(), '')
    }
    
    # 2. Replace spaces with underscores (optional, but good for Docker/Linux)
    $cleanName = $cleanName -replace ' ', '_'
    
    # 3. Handle empty input (Default to original)
    if ([string]::IsNullOrWhiteSpace($cleanName)) {
        Write-Host "   [!] No name provided, using original filename" -ForegroundColor Yellow
        $newName = $originalName
    } 
    else {
        # 4. Ensure extension exists
        if (-not $cleanName.EndsWith(".mp4")) {
            $newName = "$cleanName.mp4"
        } else {
            $newName = $cleanName
        }
    }
    ...
    Write-Host "   Final filename: $newName" -ForegroundColor Magenta
