param(
    [string]$WatchFolder = "F:\Games\Grand Theft Auto V\",
    [string]$DockerContainer = "n8n-video",
    [string]$DockerPath = "/home/node/gta5/",
    [int]$WaitSeconds = 20
)
...
Write-Host "Files will be renamed with 4 custom text prompts" -ForegroundColor Magenta
...
$action = {
    ...
    Write-Host "`n[INPUT] Enter text for filename (4 prompts required):" -ForegroundColor Cyan
    # UPDATED: Changed display string to match GTA 5 request
    Write-Host "   Format: Grand Theft Auto @V@ #Enhanced Edition# vehicle %<1>% !<2>! (<3>) .Garage. ,gta5, _Vice City_ -<4>-" -ForegroundColor Gray
    
    $invalidChars = [IO.Path]::GetInvalidFileNameChars()
    
    Write-Host -NoNewline "`n   First Prompt (%%...%%): " -ForegroundColor Yellow
    $prompt1 = Read-Host
    $clean1 = $prompt1
    foreach ($char in $invalidChars) {
        $clean1 = $clean1.Replace($char.ToString(), '')
    }
    $clean1 = $clean1 -replace ' ', '_'
    if ([string]::IsNullOrWhiteSpace($clean1)) {
        $clean1 = "prompt1"
    }
    
    Write-Host -NoNewline "   Second Prompt (!...!): " -ForegroundColor Yellow
    $prompt2 = Read-Host
    $clean2 = $prompt2
    foreach ($char in $invalidChars) {
        $clean2 = $clean2.Replace($char.ToString(), '')
    }
    $clean2 = $clean2 -replace ' ', '_'
    if ([string]::IsNullOrWhiteSpace($clean2)) {
        $clean2 = "prompt2"
    }
    
    Write-Host -NoNewline "   Third Prompt ((...)):  " -ForegroundColor Yellow
    $prompt3 = Read-Host
    $clean3 = $prompt3
    foreach ($char in $invalidChars) {
        $clean3 = $clean3.Replace($char.ToString(), '')
    }
    $clean3 = $clean3 -replace ' ', '_'
    if ([string]::IsNullOrWhiteSpace($clean3)) {
        $clean3 = "prompt3"
    }
    
    Write-Host -NoNewline "   Fourth Prompt (-...-): " -ForegroundColor Yellow
    $prompt4 = Read-Host
    $clean4 = $prompt4
    foreach ($char in $invalidChars) {
        $clean4 = $clean4.Replace($char.ToString(), '')
    }
    $clean4 = $clean4 -replace ' ', '_'
    if ([string]::IsNullOrWhiteSpace($clean4)) {
        $clean4 = "prompt4"
    }
    
    # UPDATED: Changed filename construction to match GTA 5 request
    $newName = "Grand Theft Auto @V@ #Enhanced Edition# vehicle %${clean1}% !${clean2}! (${clean3}) .Garage. ,gta5, _Vice City_ -${clean4}-.mp4"
    
    Write-Host "`n   New filename: $newName" -ForegroundColor Magenta
